...
def main():
    ...
    filtered_samples = [(path, label) for path, label in full_dataset.samples if is_valid_image(path)]
    full_dataset.samples = filtered_samples
    full_dataset.imgs = filtered_samples

    print(f"Loaded dataset with {len(full_dataset)} valid images")
    print(f"Number of classes: {len(class_names)}")

    # ─────────────────────────────────────────────────────────────
    # EXTRACT LABELS + GROUPS (CASE ID)
    # ─────────────────────────────────────────────────────────────
    def extract_case_id(path):
        filename = os.path.basename(path)
        return filename.split('-')[0]   # e.g., AC002

    all_paths  = [s[0] for s in full_dataset.samples]
    all_labels = np.array([s[1] for s in full_dataset.samples])
    all_groups = np.array([extract_case_id(p) for p in all_paths])
    all_indices = np.arange(len(all_paths))

    # ─────────────────────────────────────────────────────────────
    # GROUP-AWARE HOLD-OUT TEST SPLIT (~20%)
    # ─────────────────────────────────────────────────────────────
    sgkf_outer = StratifiedGroupKFold(n_splits=N_FOLDS, shuffle=True, random_state=42)

    outer_splits = list(sgkf_outer.split(all_indices, all_labels, all_groups))
    ...
    trainval_groups  = all_groups[trainval_idx]

    test_subset = Subset(full_dataset, test_indices.tolist())

    print(f"Hold-out test set size : {len(test_subset)}")
    print(f"Train+val pool size    : {len(trainval_indices)}")
    ...
    for model_name, fm_model, feature_dim, model_type in model_configs:
        ...
        for fold_idx, (rel_train_idx, rel_val_idx) in enumerate(
            sgkf.split(trainval_indices, trainval_labels, trainval_groups), start=1
        ):
