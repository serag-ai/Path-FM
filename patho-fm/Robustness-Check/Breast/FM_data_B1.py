...
def main():
    ...
    data_train_path = os.path.join(main_path, "B1")
    external_datasets = {
            "B3": os.path.join(main_path, "B3"),
        }

    # Load train (B1)
    train_full_dataset = datasets.ImageFolder(root=data_train_path, transform=None, is_valid_file=valid_file)
    print(f"Training dataset (B1) classes: {len(train_full_dataset.classes)}")
    ...
    print(f"Train size (data1): {len(train_dataset)}")
    print(f"Val size (data1):   {len(val_dataset)}")
    ...
    for model_name, fm_model, feature_dim, model_type in model_configs:
        ...
        csv_path = "FM_cross_B1_results.csv"
