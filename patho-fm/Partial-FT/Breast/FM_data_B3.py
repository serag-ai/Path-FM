    ...
    for model_name, fm_model, feature_dim, model_type in model_configs:
        ...
        for fold_idx, (rel_train_pid_idx, rel_val_pid_idx) in enumerate(
            ...
            train_loader = DataLoader(
                TransformedDataset(Subset(full_dataset, abs_train_idx.tolist()), model_transform),
                batch_size=32, shuffle=True, num_workers=4, pin_memory=True
            )
            val_loader = DataLoader(
                TransformedDataset(Subset(full_dataset, abs_val_idx.tolist()), model_transform),
            ...
            model = ClassificationModel(
                fm_model.to(device), feature_dim, model_type=model_type).to(device)
