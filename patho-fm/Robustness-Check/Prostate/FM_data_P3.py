...
# Phikon transform - more robust initialization
def get_phikon_transform():
    try:
        ...
        return transform  # Fallback to standard ImageNet
...
def main():
    ...
    train_full_dataset = ExcelLabeledDataset3(excel_path, image_folder, transform=None)
    print(len(train_full_dataset))           # Number of valid images

    # Split P3 into train/val (e.g., 80/20)
    ...
    print(f"Train size (P3): {len(train_dataset)}")
    print(f"Val size (P3):   {len(val_dataset)}")

    # ----------------------------
    # 2. Load external datasets (custom logic per dataset)
    # ----------------------------
    def load_external_dataset(name, data_path, transform):
        ...
        if name == "P2":
            ...
            # Create the full dataset
            full_dataset = ExcelLabeledDataset2(excel_path, image_folder, transform=None)
            return full_dataset
        elif name == "P1":
    ...
    for model_name, fm_model, feature_dim, model_type in model_configs:
        ...
        for test_name, dataset_dict in test_datasets_per_model.items():
            ...
            class_names = [col for col in df.columns if col != 'image_name']
        ...
        csv_path = "FM_cross_P3_results.csv"
